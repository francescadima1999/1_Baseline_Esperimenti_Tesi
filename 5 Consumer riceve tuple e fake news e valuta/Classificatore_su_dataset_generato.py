...
class UltraOptimizedClassifier:
    ...
    def _calculate_enhanced_keyword_bonus(self, text):
        """Calcola bonus keywords ultra-migliorato"""
        text_lower = text.lower()
    ...
    def _combine_title_and_text(self, title, text):
        """Combina titolo e testo per la classificazione"""
        # Pulisci e normalizza
        title_clean = str(title).strip() if pd.notna(title) else ""
        text_clean = str(text).strip() if pd.notna(text) else ""
        
        # Se abbiamo sia titolo che testo, dai più peso al titolo
        if title_clean and text_clean:
            # Ripeti il titolo 3 volte per dargli più importanza
            combined = f"{title_clean} {title_clean} {title_clean} {text_clean}"
        elif title_clean:
    ...
    def classify_dataset(self, test_file_path=None):
        print("\n" + "="*70)
        print("CLASSIFICAZIONE NOTIZIE")
        print("="*70)
        
        # Carica dataset
        if not test_file_path:
            test_file_path = self._get_file_path("Seleziona il file di test con notizie")
        ...
        print(f"Colonne presenti: {test_df.columns.tolist()}")
        
        # Verifica struttura file (ID, TITLE, TEXT, CAMPAGNA)
        expected_columns = {'ID', 'TITLE', 'TEXT', 'CAMPAGNA'}
        actual_columns = set([col.upper() for col in test_df.columns])
        
        # Mappa le colonne (case-insensitive)
        column_mapping = {}
        for expected in expected_columns:
            for actual in test_df.columns:
                if actual.upper() == expected:
                    column_mapping[expected] = actual
                    break
        
        missing_columns = expected_columns - set(column_mapping.keys())
        if missing_columns:
            print(f"ATTENZIONE: Colonne mancanti: {missing_columns}")
            print(f"Colonne trovate: {list(column_mapping.values())}")
            
            # Prova a inferire colonne mancanti
            if 'TEXT' not in column_mapping:
                text_candidates = ['text', 'content', 'body', 'message']
            ...
            if 'TITLE' not in column_mapping:
                ...
                for col in test_df.columns:
                    if col.lower() in title_candidates:
                        column_mapping['TITLE'] = col
                        break
            
            if 'CAMPAGNA' not in column_mapping:
                campaign_candidates = ['campagna', 'campaign', 'category', 'label']
                for col in test_df.columns:
                    if col.lower() in campaign_candidates:
                        column_mapping['CAMPAGNA'] = col
                        break
                        
            if 'ID' not in column_mapping:
                column_mapping['ID'] = test_df.columns[0]  # Prima colonna come ID
        ...
        results_df['COMBINED_TEXT_LENGTH'] = 0
        
        if 'CAMPAGNA' in column_mapping:
        ...
        for idx, row in test_df.iterrows():
            try:
                ...
                if 'TITLE' in column_mapping:
                    title = str(row[column_mapping['TITLE']]).strip() if pd.notna(row[column_mapping['TITLE']]) else ""
                
                if 'TEXT' in column_mapping:
                    text = str(row[column_mapping['TEXT']]).strip() if pd.notna(row[column_mapping['TEXT']]) else ""
                
                # Combina title e text
                combined_text = self._combine_title_and_text(title, text)
                results_df.at[idx, 'COMBINED_TEXT_LENGTH'] = len(combined_text)
                ...
                # Classifica
                result = self.classify_text(combined_text)
                ...
                results_df.at[idx, 'IS_TWITTER'] = self._is_twitter_content(combined_text)
                ...
                # Verifica accuratezza se abbiamo la colonna CAMPAGNA
                if 'CAMPAGNA' in column_mapping:
                    true_campaign = str(row[column_mapping['CAMPAGNA']]).strip() if pd.notna(row[column_mapping['CAMPAGNA']]) else ""
        ...
        avg_length = results_df['COMBINED_TEXT_LENGTH'].mean()
        print(f"Lunghezza media testo combinato: {avg_length:.0f} caratteri")
        ...
        # Analisi per presenza di TITLE vs solo TEXT
        if 'TITLE' in column_mapping and 'TEXT' in column_mapping:
            has_both = 0
            has_title_only = 0
            has_text_only = 0
            
            for idx, row in test_df.iterrows():
                title_present = pd.notna(row[column_mapping['TITLE']]) and str(row[column_mapping['TITLE']]).strip()
                text_present = pd.notna(row[column_mapping['TEXT']]) and str(row[column_mapping['TEXT']]).strip()
                
                if title_present and text_present:
                    has_both += 1
                elif title_present:
                    has_title_only += 1
                elif text_present:
            ...
            print(f"  Con TITLE e TEXT: {has_both}")
            print(f"  Solo TITLE: {has_title_only}")
            print(f"  Solo TEXT: {has_text_only}")
        
        # Salva risultati
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = f"risultati_notizie_classificate_{timestamp}.xlsx"
        ...
        try:
            ...
            csv_file = f"risultati_notizie_classificate_{timestamp}.csv"
    ...
    def analyze_classification_errors(self, results_df):
        ...
        # Trova il nome della colonna campagna vera
        campaign_col = None
        for col in results_df.columns:
            if col.upper() == 'CAMPAGNA':
                campaign_col = col
                break
        
        if not campaign_col:
            print("Colonna CAMPAGNA non trovata")
            return
        
        # Errori per campagna vera
        errors_by_true_campaign = results_df[
            results_df['CAMPAIGN_PREDICTION_CORRECT'] == False
        ].groupby(campaign_col).size().sort_values(ascending=False)
        ...
        top_campaigns = results_df[campaign_col].value_counts().head(5).index
        
        print(f"\nMatrice confusione (top 5 campagne):")
        print("Vera vs Predetta")
        
        for true_camp in top_campaigns:
            true_subset = results_df[results_df[campaign_col] == true_camp]
        ...
        # Analisi errori per lunghezza testo
        error_by_length = results_df[results_df['CAMPAIGN_PREDICTION_CORRECT'] == False]['COMBINED_TEXT_LENGTH'].describe()
        correct_by_length = results_df[results_df['CAMPAIGN_PREDICTION_CORRECT'] == True]['COMBINED_TEXT_LENGTH'].describe()
        
        print(f"\nLunghezza testo - Errori vs Corretti:")
        print(f"  Errori - Media: {error_by_length['mean']:.0f}, Mediana: {error_by_length['50%']:.0f}")
        print(f"  Corretti - Media: {correct_by_length['mean']:.0f}, Mediana: {correct_by_length['50%']:.0f}")
    
    def generate_optimization_report(self, results_df):
        """Genera report di ottimizzazione dettagliato per notizie"""
        report = []
        report.append("="*80)
        report.append("REPORT CLASSIFICATORE NOTIZIE")
        report.append("="*80)
        ...
        report.append(f"\nSTATISTICHE GENERALI:")
        report.append(f"  Dataset totale: {total} notizie")
        ...
        if 'CAMPAIGN_PREDICTION_CORRECT' in results_df.columns:
            method_performance['accuracy'] = results_df.groupby('CLASSIFICATION_METHOD')['CAMPAIGN_PREDICTION_CORRECT'].mean() * 100
        
        report.append(f"\nPERFORMANCE PER METODO:")
        for method, stats in method_performance.iterrows():
            method_total = len(results_df[results_df['CLASSIFICATION_METHOD'] == method])
            if 'accuracy' in stats:
                report.append(f"  {method}: {stats['accuracy']:.1f}% accuracy, {stats['PREDICTION_CONFIDENCE']:.3f} avg confidence ({method_total} notizie)")
            else:
                report.append(f"  {method}: {stats['PREDICTION_CONFIDENCE']:.3f} avg confidence ({method_total} notizie)")
        ...
        # Analisi lunghezza testo
        if 'COMBINED_TEXT_LENGTH' in results_df.columns:
            length_stats = results_df['COMBINED_TEXT_LENGTH'].describe()
            report.append(f"\nANALISI LUNGHEZZA TESTO:")
            report.append(f"  Media: {length_stats['mean']:.0f} caratteri")
            report.append(f"  Mediana: {length_stats['50%']:.0f} caratteri")
            report.append(f"  Min/Max: {length_stats['min']:.0f}/{length_stats['max']:.0f} caratteri")
        ...
        if 'CAMPAIGN_PREDICTION_CORRECT' in results_df.columns:
            ...
            if campaign_col:
                campaign_accuracy = results_df.groupby(campaign_col).agg({
        ...
        # Analisi Twitter vs Non-Twitter
        if 'IS_TWITTER' in results_df.columns:
            twitter_stats = results_df.groupby('IS_TWITTER').agg({
                'PREDICTION_CONFIDENCE': 'mean'
            })
            
            if 'CAMPAIGN_PREDICTION_CORRECT' in results_df.columns:
                twitter_stats['accuracy'] = results_df.groupby('IS_TWITTER')['CAMPAIGN_PREDICTION_CORRECT'].mean() * 100
            
            report.append(f"\nPERFORMANCE TWITTER vs NON-TWITTER:")
            for is_twitter, stats in twitter_stats.iterrows():
                content_type = "Twitter" if is_twitter else "Non-Twitter"
                count = len(results_df[results_df['IS_TWITTER'] == is_twitter])
                if 'accuracy' in stats:
                    report.append(f"  {content_type}: {stats['accuracy']:.1f}% accuracy, {stats['PREDICTION_CONFIDENCE']:.3f} avg confidence ({count} notizie)")
                else:
                    report.append(f"  {content_type}: {stats['PREDICTION_CONFIDENCE']:.3f} avg confidence ({count} notizie)")
        ...
        short_texts = len(results_df[results_df['COMBINED_TEXT_LENGTH'] < 50])
        if short_texts > total * 0.1:
            report.append(f"  - Molti testi corti ({short_texts}): considera preprocessing migliore")
        
        if 'IS_TWITTER' in results_df.columns and 'CAMPAIGN_PREDICTION_CORRECT' in results_df.columns:
            twitter_df = results_df[results_df['IS_TWITTER'] == True]
            if len(twitter_df) > 0:
...
def main():
    print("CLASSIFICATORE NOTIZIE ULTRA-OTTIMIZZATO")
    print("="*60)
    print("Questo classificatore analizza notizie con struttura: ID, TITLE, TEXT, CAMPAGNA")
    print("="*60)
